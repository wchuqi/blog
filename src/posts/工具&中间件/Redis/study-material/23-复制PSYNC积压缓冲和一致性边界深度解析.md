---
title: "Redis学习资料：复制、PSYNC、积压缓冲和一致性边界深度解析"
date: 2026-06-20
description: "Redis 复制要解决从节点如何追上主节点。首次连接通常需要全量同步，主节点生成 RDB 发给从节点，同时缓存增量写命令。断线重连时，如果主节点还保留从节点缺失的增量复制流，就可以部分同步；否则只能全量同步。PSYNC 使用 runid 和…"
tags:
  - "工具&中间件"
  - "Redis"
  - "深度解析"
